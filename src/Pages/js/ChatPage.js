...
import girl4 from "../../assets/Dummyimages/Girl6.jpg";
...
const  initailConection=JSON.parse( window.localStorage.getItem("currentChatConnectionId"))
   setCurrentSplit({
...
const DeleteMessage=(key)=>{
  console.log(key)
  if( result === 1){
    const chatsRef = ref(database, "Chatrooms/" +  currentSplit.id +',' +currentConnection.id + '/'  +key);
     remove(chatsRef)
  }
  if(result === -1){
    
    const chatsRef= ref(database, "Chatrooms/" + currentConnection.id + ','  + currentSplit.id  + '/'  +key);
    remove(chatsRef)
  } 
}
      ...
      <div className="chatpage__container" >
        ...
        <div className="chatpage__topnav">
          <div className="chatpage__topnav-top">
            <img
              ...
              src={menu}
            ></img>
            <img className="chatpage__topnav-top-profileimg" src={girl} onClick={()=>Navigate(`/profile/${auth.uid}`)}></img>
          </div>
        ...
        <Breakpoint customQuery="(min-width: 1200px)">
          <div className="chatpage__connections">
            ...
            { currentSplit.connections.length === 0  && <p className="chatpage-noconnectionmessage"> This Split has no connections</p>
        ...
        <div ref={messageDivRef} className="chatarea" >
            ...
            {messages.map(message=>
              <Message  onDeleteMessage={DeleteMessage}  key={message.messageId} senderId={message.Sender} messageId={message.messageId} currentSplitId={currentSplit.id} >{message.text} </Message>
              )}
                <div ref={messagesEndRef} />
        </div>

        <ChatForm currentSplitId={currentSplit.id} currentConnectionId={currentConnection.id}></ChatForm>
